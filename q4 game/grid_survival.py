...
class GridApp(tk.Tk):
    def __init__(self):
        ...
        self.bolt_frames = []
        self._bolt_frame_index = 0
        self._bolt_animation_job = None
    ...
    def _build_ui(self):
        ...
        bolt_path = os.path.join(os.path.dirname(__file__), "bolt.gif")
        if os.path.exists(bolt_path):
            try:
                img = tk.PhotoImage(file=bolt_path)
                target_width = 100
                ...
                self.bolt_image = None
        self.header_right = tk.Frame(self.header, bg="#0b1220")
        self.header_right.pack(side="right", padx=10, pady=6)
        ...
        if os.path.exists(bolt_path):
            try:
                ...
                target_width = 80
                while True:
                    frame = tk.PhotoImage(file=bolt_path, format=f"gif -index {index}")
                    if frame.width() > target_width:
                        factor = max(1, frame.width() // target_width)
                        frame = frame.subsample(factor, factor)
                    frames.append(frame)
                    index += 1
            except tk.TclError:
                frames = []
            self.bolt_frames = frames

        if self.bolt_frames:
            self.bolt_label = tk.Label(self.header_right, image=self.bolt_frames[0], bg="#0b1220")
            self.bolt_label.pack(anchor="ne")
            if len(self.bolt_frames) > 1:
                self._bolt_frame_index = 0
                self._animate_bolt()
        else:
    ...
    def _animate_bolt(self):
        if not self.bolt_frames:
            return
        self._bolt_frame_index = (self._bolt_frame_index + 1) % len(self.bolt_frames)
        self.bolt_label.config(image=self.bolt_frames[self._bolt_frame_index])
        self._bolt_animation_job = self.after(120, self._animate_bolt)
